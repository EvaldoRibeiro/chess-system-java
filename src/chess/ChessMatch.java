...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if (testCheck(currentPlayer)) {
			...
			throw new ChessException("Voce nao pode ser colocar em ckeck"); // "You can't put yourself in check"
		}
	...
	private ChessPiece king(Color color) {
		...
		throw new IllegalStateException("Nao existe o " + color + "Rei no tabuleiro"); // "There is not" + color + "King
																						// on the board"
	}
			...
			for (int i = 0; i < board.getRows(); i++) {
				for (int j = 0; j < board.getColumns(); j++) {
					if (mat[i][j]) {
						...
						Piece CapturaPeca = makeMove(origem, destino);
